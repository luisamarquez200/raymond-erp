                    ...
                    <Table className="w-full text-xs">
                        <TableHeader className="bg-slate-50/50 sticky top-0 z-10 border-b border-slate-100">
                            <TableRow className="hover:bg-transparent">
                                ...
                                <TableHead className="py-2.5 px-4 text-right font-semibold text-slate-400 uppercase text-[11px] tracking-wider whitespace-nowrap">P. Mes</TableHead>
                                <TableHead className="py-2.5 px-4 text-right font-semibold text-slate-400 uppercase text-[11px] tracking-wider whitespace-nowrap">Acumulado</TableHead>
                                <TableHead className="py-2.5 px-4 text-right font-semibold text-slate-400 uppercase text-[11px] tracking-wider whitespace-nowrap">Total</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {data.length === 0 ? (
                                ...
                                data.map((row, idx) => (
                                    <TableRow key={idx} className="hover:bg-slate-50/50 transition-colors border-b border-slate-100/60">
                                        <TableCell className="py-2.5 px-4 font-medium text-slate-700 whitespace-nowrap max-w-[150px] truncate" title={row.cliente}>{row.cliente}</TableCell>
                                        <TableCell className="py-2.5 px-4 text-right font-normal text-slate-500 whitespace-nowrap tabular-nums">{formatCurrency(row.presupuesto_mes)}</TableCell>
                                        <TableCell className="py-2.5 px-4 text-right font-normal text-slate-500 whitespace-nowrap tabular-nums">{formatCurrency(row.pendiente_acumulado)}</TableCell>
